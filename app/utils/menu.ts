...
export const menus: MenuItem[] = [
  ...
  },
  {
    label: 'Management',
    items: [
      { label: '회사', icon: 'pi pi-fw pi-building', to: '/management/companies' },
      { label: '팩토리', icon: 'pi pi-fw pi-book', to: '/management/factories' },
      { label: '계정', icon: 'pi pi-fw pi-users', to: '/management/accounts' },
  ...
  },
  {
    label: 'RPA',
    items: [
      { label: '작업PC', icon: 'pi pi-fw pi-desktop', to: '/rpa/workers' },
      ...
      { label: '순서정책', icon: 'pi pi-fw pi-sort-alt', to: '/rpa/business-day-order' },
      {
        label: '순서 미리보기',
        icon: 'pi pi-fw pi-eye',
        to: '/rpa/business-day-order-preview',
      },
      { label: '휴일관리', icon: 'pi pi-fw pi-calendar-times', to: '/rpa/holidays' },
    ],
  },
]
...
export const getMenuChainFindByMenuPath = (
  ...
  for (const menu of menus) {
    ...
    if (menu.items) {
      ...
      if (childPath) {
        return [
          { label: menu.label, route: menu.to || '', icon: menu.icon || '' },
          ...childPath,
        ]
      }
    }
